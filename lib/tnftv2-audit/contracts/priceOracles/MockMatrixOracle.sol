 ...
 */
contract MockMatrixOracle is ChainlinkClient, ConfirmedOwner, AutomationCompatibleInterface {
    using Chainlink for Chainlink.Request;
    ...
    function createItems(
        ...
        uint256[] calldata weSellAt,
        uint256[] calldata lockedAmount,
        uint256[] calldata weSellAtStock,
        uint16[] calldata currency,
        uint16[] calldata location
    ) public {
        uint256 length = fingerprint.length;
     ...
     */
    function updateItem(uint256 fingerprint, uint256 weSellAt, uint256 lockedAmount) public {
        Data storage data = fingerprintData[fingerprint];
        uint256 oldWeSellAt = data.weSellAt;
        uint256 oldLockedAmount = data.lockedAmount;
        ...
        // Emit PriceUpdated event
        emit PriceUpdated(fingerprint, oldWeSellAt, weSellAt, oldTimestamp, block.timestamp);
        latestPrices++;
        lastUpdateTime = block.timestamp;
        // send notification to dispatcher
        ITangibleWrapper(tangibleWrapper).notify(
            fingerprint,
            oldWeSellAt + oldLockedAmount,
            weSellAt + lockedAmount,
            data.currency
        );
    }
     ...
     */
    function updateStock(uint256 fingerprint, uint256 weSellAtStock) public {
        Data storage data = fingerprintData[fingerprint];
     ...
     */
    function oracleDataBatch(uint256[] calldata _fingerprints) public view returns (Data[] memory) {
    ...
    function setTangibleWrapperAddress(address _tangibleWrapper) public {
        tangibleWrapper = _tangibleWrapper;
    }

    function setTangibleMultiSig(address _tangibleMultiSig) public {
        tangibleMultiSig = _tangibleMultiSig;
    }

    modifier authorizeChainlink() {
        require(msg.sender == submitter || msg.sender == owner(), "Caller is not the Chainlink");
        _;
    }
    ...
    function setUpdateInterval(uint256 _updateInterval) public authorizeChainlink {
